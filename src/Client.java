...
public class Client implements Runnable {
  ...
  public static void main(String[] args) throws IOException, Exception {
    ...
    while (true) {
      ...
      if (res.equalsIgnoreCase("no")) {
        ...
        try {
          ...
          System.err.println(
            line +
            "Please enter a port number with no letters or special characters(digits only). You entered: " +
            port
          );
        }
        ...
        System.out.println(
          line + "Please enter just 'yes' or 'no'. You entered: " + res
        );
      }
      //} else {
    ...
    try {
      ...
      serverInput =
        new BufferedReader(new InputStreamReader(MyClient.getInputStream()));
    } catch (UnknownHostException e) {
      System.err.println(line + "This host is unknown: " + machineName);
    } catch (IOException e) {
      System.err.println(
        line + "Unable to get Input/output connection of host: " + machineName
      );
    }
      ...
      try {
        ...
        while (!socketClosed) {
          ...
          String encodedKey = Base64
            .getEncoder()
            .encodeToString(key.getEncoded()); //SecretKey is base64 encoded since direct string encryption gives key in string format during decryption which cant be converted to SecretKey Format
